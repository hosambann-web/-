...
client.on('message', async (message) => {
  try {
    ...
    // -------- 6ب) !اوامر - يعرض كل الأوامر المتاحة --------
    if (body === '!اوامر') {
      const commandsList = `
📋 *أوامر بوت عمي حسام*

🔹 *تعال @شخص الرسالة* — يبعت رسالة لشخص معيّن (أدمن بس)
🔹 *!توقف* — يسكت عمي حسام (الذكاء الصناعي)
🔹 *!تشغيل* — يرجّع عمي حسام
🔹 *!وتكلم* — رد ثابت من البوت
🔹 *!قفل* — يقفل الشات (أدمن بس، الأدمن بس يقدر يحكي)
🔹 *!فتح* — يفتح الشات للكل (أدمن بس)
🔹 *!باند @شخص* — يطرد عضو من الجروب (أدمن بس)
🔹 *!عمي_حسام سؤالك* — يبدأ محادثة مع عمي حسام
🔹 *رد (Reply) على رسالة عمي حسام* — يكمل معاه المحادثة من غير أمر
🔹 *!اوامر* — يعرض هاد القائمة
      `.trim();
    ...
    if (aiEnabled && activeConversations.has(convoKey) && body.length > 1 && !body.startsWith('!')) {
      const reply = await askAI(body);
      await message.reply(reply);
      return;
    }

    // -------- 8) رد (Reply) على رسالة من عمي حسام نفسه = يرد عليك بدون أمر --------
    ...
    // ملاحظة: هاد ما بيفعّلش "محادثة دائمة"، لازم كل مرة تكون Reply فعلي على رسالة البوت
