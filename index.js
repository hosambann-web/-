...
const AVATAR_SPOTS = {
  welcome: { cx: 471, cy: 629, r: 270 },
  goodbye: { cx: 474, cy: 624, r: 280 }
};
...
function violationDmBanner(reasonText, memberLine) {
  return [
    '⌬──══─┈•⤣⚡⤤•┈─══──⌬',
    '⚡⃝🌙 *سـبـب الـتـحـذيـر:*',
    `> ${reasonText}`,
    ...
    '',
    'تـم تسـجيـل الـتحـذيـر وتـم الإرسـال بالخاص، وانت خليها حلوة 🌙',
    '⌬──══─┈•⤣⚡⤤•┈─══──⌬'
  ].join('\n');
}
...
client.on('message', async (message) => {
  try {
    ...
    if (body.startsWith('!مخالفة') || body.startsWith('!مخالفه')) {
      ...
      if (reason.length === 0) {
        await message.reply('لازم تكتب سبب التحذير 📌 مثال: !مخالفة @فلان سبب التحذير');
        return;
      }
      ...
      try {
        await client.sendMessage(targetId, violationDmBanner(reason, `@${target.number}`));
      } catch (dmErr) {
