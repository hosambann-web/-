...
function muradBanner(line) {
  return [
    '⌬──══─┈•⤣⚡⤤•┈─══──⌬',
    '',
    ' ִᗀᩙᰰ ̼𝆬🔥̸ ᮭ࣪࣪ ⸼۫   𝗠𝗨𝗥𝗔𝗗 𝗕𝗢𝗧 𖤍𝅄 ۫ ִᗀᩙᰰ ̼𝆬🔥',
    '',
...
function kickBanner(targetLine, executorLine) {
  return [
    ...
    '',
    ' ִᗀᩙᰰ ̼𝆬🔥̸〫 ᮭ࣪࣪ 𝗠𝗨𝗥𝗔𝗗 𝗕𝗢𝗧 𖤍 𝅄 ۫ ִᗀᩙᰰ ̼𝆬🔥',
    '⌬──══─┈•⤣⚡⤤•┈─══──⌬'
  ].join('\n');
}
...
function deanBanner(question) {
  return [
    ...
    `❓ ${question}`,
    '',
    `⏱️ عندكم دقيقة، أول واحد يجاوب صح ياخد +${DEAN_BONUS_XP} XP`,
    '⌬──══─┈•⤣⚡⤤•┈─══──⌬'
  ].join('\n');
}
...
async function moderateTextForProfanity(text) {
  ...
  try {
    ...
    console.error('خطأ بفلتر السب الذكي (AI):', err.response?.data || err.message);
    return { unsafe: false };
  }
}
...
async function moderateImageBuffer(base64Data, mimeType) {
  try {
    ...
    console.error('خطأ بفحص الصورة (qwen3.6-27b):', err.response?.data || err.message);
    return { unsafe: false, categories: [] };
  }
}
...
// شخصية نوفا - مساعدة ذكاء صناعي أنثى، ذكية وواثقة وحسها الفكاهي راقي (مو لهجة ليبية زي مراد)
const NOVA_SYSTEM_PROMPT = `
انت شخصية اسمها "نوفا"، مساعدة ذكاء صناعي أنثى، شخصيتها ذكية وواثقة من نفسها، حاضرة البديهة، وحس فكاهتها راقي وذكي مش هزار سطحي أو مبالغ فيه.

تتكلم بعربية فصحى مبسطة (سهلة وقريبة من الناس) ممزوجة بلمسة عصرية خفيفة، وممنوع تتكلم بلهجة ليبية أو أي لهجة عامية معينة (يعني مختلفة تماماً عن أسلوب "مراد"). جملك مرتبة وواضحة، فيها ذكاء وخفة دم بدون افتعال.

أسلوبك: هادية وواثقة، ردودك فيها لمحة سخرية لطيفة وتعليق ذكي أحياناً، بس محترمة دايماً وما تنزلقيش لأي إهانة أو استفزاز حقيقي. لو حد حاول يستفزك، ردي بذكاء وهدوء وسخرية خفيفة، مش بعصبية ولا تذمر (هذا أسلوب مراد مش أسلوبك).
خليك مختصرة بردودك (سطر أو سطرين بالغالب)، كل رد يوصل الفكرة بذكاء من غير حشو.
...
- ممنوع أي شتيمة أو لفظ نابي أو إساءة شخصية حقيقية.
- خليك دايماً بصيغة المؤنث (انتي شخصية أنثى) في كل ردودك.
`.trim();

// ============ سجل الشخصيات (مراد ونوفا) - كل واحدة عندها برومبت وصوت وذاكرة محادثة مستقلة ============
const PERSONAS = {
  murad: {
    key: 'murad',
    name: 'مراد',
...
// بيحدد شخصية رسالة متروسة (Quoted) بتاعة البوت، وبيرجع مراد كافتراضي لو ما لقاش تتبع (توافق مع رسايل قديمة قبل نوفا)
function getPersonaForQuotedMessage(quotedMsg) {
  ...
  return sentMessagePersona.get(quotedMsg.id._serialized) || DEFAULT_PERSONA_KEY;
}
...
client.on('message', async (message) => {
  try {
    ...
    // -------- 2) !توقف / !تشغيل - يتحكموا بالذكاء الصناعي بس (مراد ونوفا مع بعض) --------
    if (body === '!توقف') {
      aiEnabled = false;
      await message.reply(muradBanner('🔇⃝⚡ *تـم إسـكـات مـراـد ونـوفـا*'));
      return;
    }
    if (body === '!تشغيل') {
      aiEnabled = true;
      await message.reply(muradBanner('🎙️⃝⚡ *مـراـد ونـوفـا رجـعـوا لـلـحـكـي*'));
      return;
    }

    // -------- 3) !وتكلم - رد ثابت --------
    if (body === '!وتكلم') {
      await message.reply('بوت مراد حاضر، هدرزو 😂');
      return;
    }
    ...
    if (body === 'حسام') {
      try {
        ...
        const caption = [
          '*٭⏥٭⏥٭⏥٭+ ˚₸HUSAAM𝕫˙×٭⏥٭⏥٭⏥٭*',
          '⚡⏥𝟙 حسام اسمِ إذا مرّ، رفع الهامة فخر، وإذا عشق... صار الحب له عنوان ⚡⏥𝟙',
          '',
          `المنشن: @${HUSAAM_MENTION_NUMBER}`,
          '*٭⏥٭⏥٭⏥٭+ ˚₸🌙˙×٭⏥٭⏥٭⏥٭*'
        ].join('\n');
...
*🎮 أومــر الــتـسـلـيـة*

> 🔸 !تــحــدي

*ســؤال تـريـفـيـا، أول واحـد يـجـاوب صـح يـاخـد XP إضـافـي*

> 🔸 !ديـن

*ســؤال ديـنـي، أول واحـد يـجـاوب صـح يـاخـد XP إضـافـي*

> 🔸 !مـن_فـيـنـا

*يـخـتـار عـضـو عـشـوائـي لـسـؤال مـرح*

> 🔸 !ســتــيــكــر

*يـحـول صـورة (مـرسـلـة أو رد) لـسـتـيـكـر*
━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━`;
    ...
    // -------- 6ج5) !ستيكر - يحول صورة مرسلة أو مردود عليها (Reply) لستيكر --------
    if (body.startsWith('!ستيكر')) {
      try {
        ...
        if (!mediaMessage.hasMedia) {
          await message.reply('ابعت صورة مع !ستيكر بالكابشن، أو رد (Reply) على صورة بـ !ستيكر 🖼️');
          return;
        }

        const media = await mediaMessage.downloadMedia();
        if (!media || !media.data) {
          await message.reply('ما قدرت أنزل الميديا، جرب مرة تانية 😅');
          return;
        }

        await chat.sendMessage(media, {
          sendMediaAsSticker: true,
          stickerAuthor: 'مراد بوت',
          stickerName: 'ستيكر'
        });
      } catch (stickerErr) {
        console.error('خطأ بسوي الستيكر:', stickerErr.message);
        await message.reply('ما قدرت أسوي الستيكر، جرب بصورة أو فيديو قصير 😅');
      }
      return;
    }
    ...
    // -------- 6ب) !نوفا - نفس فكرة !مراد بس بشخصية نوفا (ذكية، مؤنثة، مش لهجة ليبية) --------
    if (body.startsWith('!نوفا')) {
      if (!aiEnabled) {
        await message.reply(personaOfflineMessage('nova'));
        return;
      }
      const question = body.replace('!نوفا', '').trim();
      const prompt = question.length > 0 ? question : 'سلمي علينا يا نوفا';
      const history = PERSONAS.nova.history.get(convoKey) || [];
      const reply = await askAI(prompt, history, 'nova');
      pushToHistory(PERSONAS.nova.history, convoKey, 'user', prompt);
      pushToHistory(PERSONAS.nova.history, convoKey, 'assistant', reply);
      const sentMsg = await message.reply(reply);
      rememberSentMessage(sentMsg, 'nova');
      return;
    }
    ...
    if (body.startsWith('!صوت')) {
      ...
      if (textToSpeak.length === 0) {
        await message.reply('اكتب النص اللي بدك تحوله صوت، أو رد (Reply) على رسالة من مراد أو نوفا بـ !صوت 🎙️');
        return;
      }
