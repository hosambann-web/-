...
process.on('unhandledRejection', (reason) => {
  ...
  console.error('name:', reason && reason.name);
  console.error('message:', reason && reason.message);
  console.error('stack:', (reason && reason.stack) || util.inspect(reason, { depth: 5 }));
...
process.on('uncaughtException', (err) => {
  ...
  console.error('stack:', (err && err.stack) || util.inspect(err, { depth: 5 }));
  console.error('=== UNCAUGHT EXCEPTION END ===');
});

// ============ إعدادات عامة ============
const GROQ_API_KEY = process.env.GROQ_API_KEY;
// رجعنا نعتمد بالكامل على Groq (الشات + فلتر السب + فلتر الصور + الصوت بملف voice.js)
// بعد ما صار عندنا مشكلة بمفتاح Gemini. خلينا متغير GEMINI_API_KEY موجود بس ما بيتستخدمش حاليًا
// (لو حبيت ترجع تستخدم Gemini بالمستقبل، المتغير لسا موجود بالـ.env)
const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
const GROQ_CHAT_ENDPOINT = 'https://api.groq.com/openai/v1/chat/completions';
const GROQ_CHAT_MODEL = 'llama-3.3-70b-versatile'; // موديل الشات النصي (مراد/نوفا + فلتر السب)
const GROQ_VISION_MODEL = 'meta-llama/llama-4-scout-17b-16e-instruct'; // موديل الرؤية (فلتر الصور)
...
const MENU_GIF_FILENAME = 'f01e4b59c072d8857f22be2a6a9a55b9.gif';
...
const BAD_WORDS = [
  ...
  // ألفاظ سب أجنبية (انجليزي) شائعة - إضافة (طلب حماية الجروب من السب بأي لغة)
  'fuck', 'fucking', 'fucker', 'motherfucker', 'fuck you', 'fuck u',
  'shit', 'bullshit', 'bitch', 'bastard', 'asshole', 'dick', 'dickhead',
  'pussy', 'whore', 'slut', 'cunt', 'cock', 'wanker', 'twat',
  'son of a bitch', 'stfu', 'retard', 'idiot fuck'
];
        ...
        messages: [
          {
            role: 'system',
            content: [
              'أنت مصنّف أمان صارم لرسائل جروب واتساب فيه بنات، هدفك حماية الأعضاء من السب والقذف والإهانات الجنسية بأي لغة كانت (عربي، انجليزي، فرنسي، أو أي لغة/لهجة تانية، حتى لو مخلوطة بين لغتين بنفس الرسالة).',
              'هيدي الرسالة ممكن تكون مكتوبة بطريقة عادية أو مموّهة/مشفرة عشان تتهرب من فلتر كلمات ثابت، مثلاً:',
              '- حروف متباعدة بمسافات أو نقط أو رموز (ق ح ب ة / ق.ح.ب.ة أو f u c k / f.u.c.k)',
              '- أرقام أو حروف انجليزي بدل حروف عربي أو العكس (q7ba, 3ahra, a55, sh1t)',
              '- تكرار حروف (قحححبة أو fuuuuck)',
              '- الكلمة ملفوفة جوه جملة طويلة أو بصيغة غير مباشرة لكن قصدها واضح إهانة أو سب جنسي، بأي لغة',
...
async function moderateImageBuffer(base64Data, mimeType) {
  try {
    ...
    console.error('خطأ بفحص الصورة (Groq vision):', err.response?.data || err.message);
    return { unsafe: false, categories: [] };
  }
}
...
// ============ دالة الاتصال بـ Groq (الشات الرئيسي - مراد/نوفا) ============
...
const client = new Client({
  ...
  // ============ تثبيت نسخة واتساب ويب (مهم جداً حاليًا) ============
  // المشكلة: بدون هاد الإعداد، البوت كان بياخذ "آخر نسخة" من موقع واتساب ويب تلقائيًا
  // كل مرة يشتغل - وآخر نسخة حاليًا (تقريبًا 2026) فيها Bug معروف وموثّق بمكتبة whatsapp-web.js
  // بيخلي getChatById/message.getChat() يرمي خطأ داخلي مشفّر مع كل رسالة (نفس مشكلة "r" اللي كانت عم تصير).
  // الحل: نجبر البوت يستخدم نسخة أقدم وأثبت من الموقع، بدل أحدث نسخة مكسورة.
  // لو هاي النسخة كمان طلعت فيها مشاكل، جرب رقم نسخة تاني أقدم شوي من:
  // https://github.com/wppconnect-team/wa-version/tree/main/html
  webVersionCache: {
    type: 'remote',
    remotePath:
      'https://raw.githubusercontent.com/wppconnect-team/wa-version/main/html/2.3000.1023204141.html'
  }
});
...
client.on('group_join', async (notification) => {
  // ميزة الترحيب معطّلة بالكامل (تعطيل يدوي بالكود) - لو بدك ترجعها، شيل سطر الـ return التالي
  return;
  try {
...
client.on('group_leave', async (notification) => {
  // ميزة الوداع معطّلة بالكامل (تعطيل يدوي بالكود) - لو بدك ترجعها، شيل سطر الـ return التالي
  return;
  try {
    const chat = await notification.getChat();
    const contacts = await notification.getRecipients();
...
client.on('message', async (message) => {
  try {
    // مهم جداً: message.getChat() بيستدعي getChatById من جوا، واللي حاليًا بيفشل بشكل ثابت
    // مع حسابات فيها نظام الخصوصية الجديد (LID) - مشكلة معروفة وموثّقة بمكتبة whatsapp-web.js
    // نفسها (لسا ما انصلحت من المطورين وقت كتابة هاد الكود).
    // الحل: لو فشل الجلب الطبيعي، منبني كائن "شات مبسّط" بديل يغطي أهم العمليات
    // (إرسال رسالة، معرفة إذا جروب) عشان الأوامر الأساسية تضل شغالة بدل ما توقف كلها.
    let chat;
    ...
    try {
      ...
      console.error('تحذير: فشل جلب كائن الشات الحقيقي (مشكلة LID المعروفة)، هستخدم نسخة مبسّطة بديلة. السبب:', chatErr.message);
      chat = {
...
> *ᴘᴏᴡᴇʀᴇᴅ:* *Hossam Bot*`;
    ...
    // بنطبع تفاصيل الخطأ بسطور إنجليزي منفصلة (بدون خلط عربي بنفس السطر) عشان نتفادى
    // مشكلة عرض bidi اللي كانت بتقص الرسالة وتوريها كأنها حرف "r" بس على لوق Railway
    console.error('=== MSG ERROR START ===');
    console.error('name:', err && err.name);
    console.error('message:', err && err.message);
    console.error('stack:', (err && err.stack) || util.inspect(err, { depth: 5 }));
    console.error('full:', util.inspect(err, { depth: 6, showHidden: false }));
    console.error('=== MSG ERROR END ===');
